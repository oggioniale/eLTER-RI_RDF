...
show_help() {
  ...
  echo "1. Fetches a list of sites from the DEIMS API and processes them based on the date of changes."
  echo "2. Generates RDF data for sites and networks using SPARQL-Generate."
  echo "3. Compares new data with previously processed data using SHA256 checksums."
  echo "4. If changes are detected, it updates the corresponding files on the web server and loads them into a Fuseki triple store."
  echo "5. Extracts relevant URIs from the generated TTL files and deletes outdated triples in the triple store."
  echo
  echo "Options:"
